...
| **インボイス未登録の支払先に2%ロイヤリティ控除（請求書・分配明細）** | ✅ 完了・本番稼働中（PR #3マージ `7575ef5`、マイグレ046適用済、Vercelデプロイ成功）。ダミー202606で③と銭単位一致を実測 |

---

## 💴 インボイス未登録2%ロイヤリティ控除（2026-07-13 実装 → 2026-07-13 本番リリース済み）

発注元ルール（2026-07-09 ダミー計算資料）「インボイス登録がない方については2％のロイヤリティを差し引く」への対応。

- **実装（ブランチ `feat/invoice-royalty-2pct`）**
  - マイグレ **046**: scouts/managers に `invoice_registration_number`（T+13桁・agencies 012 と同形式）追加。invoices に `royalty_rate`/`royalty_deduction_jpy`、distributions に `royalty_deduction_jpy` 追加。`calc_royalty_deduction()`（単一計算口・2%）新設、`recalculate_distributions` を控除対応で再定義（manager/agency 分の控除は total_side 残差へ戻し **元本一致 Σamount=gross を維持**。scout は別軸で控除のみ）。
  - TS: `INVOICE_ROYALTY_RATE=0.02`（constants）。請求書は税込総額から2%控除→税抜/税分解（invoices.ts）。プレビュー/詳細ダイアログ・請求書PDF注記・分配明細画面に控除表示。pgTAP期待値を控除込みに更新（plan 15）。
- **検証（2026-07-13・ローカルPG16）**: 001→046 全適用OK。ユニット10アサートPASS（控除・元本一致・冪等・非admin拒否）。ダミー計算資料49名のE2E再現で③請求書金額計算と**銭単位で一致**（大西皇3,278.60／CANDY 7,572.01／KIKIYO 477.93／杉谷1,194.83／MG 37,597.16。請求書はVENKEI 8,392円・KOOL 30,667円＝Excel比±0.35円以内の整数円丸めのみ）。`tsc`/`eslint`/`next build` 通過。
- **リリース（実施済み・2026-07-13）**
  1. ✅ 本番へマイグレ046適用（読み取り検証で新列4テーブル＋`calc_royalty_deduction`動作確認）
  2. ✅ PR #3 マージ（`7575ef5`）→ Vercelデプロイ成功
  3. ✅ マスタ投入: KIKIYOスカウトに登録番号投入済み（ききよ代理店番号を流用）。大西皇・CANDY大木・VENKEIは未登録＝2%控除が正。
- **マスタの現状（2026-07-25 確認）**: scouts=KIKIYOのみ登録済／大西皇・CANDY大木・杉谷=未登録。managers 杉谷=未登録。**トータルサイド（杉谷 MG40%・しのえり分）は9月にインボイス番号発行予定→それまで2%控除。発行後に番号登録＋該当月を `recalculate_distributions` で再計算すれば遡及して控除が外れる**（同一番号を複数担当者に設定するのはシステム上OK）。
- **留意点**: 過去月は自動では再計算されない（レート変更等で再生成した場合のみ新ルールが適用される＝遡及に注意）。
- **ダミーテスト（2026-07-14）**: 本番へダミー202606取込（rate159.31）、分配明細を実測し③と照合済み（差は利上訂正・トータルサイド2%・円丸めの3要因のみ）。**本番運用時は6月実CSVで上書き取込が必要／VENKEI率0.25はテスト値で要修正／杉谷メールは仮アドレス**。杉谷さんの manager_user ログインは発行済み。
- **実6月データ取込（2026-07-29）**: 発注元が実6月CSV取込（rate161.15・45名・ダイヤ216,559）。CANDYのダイヤが59不足（10,792→正10,851）と判明。原因＝ダミー準備時に彩愛(ascan_0501)をトータルサイドに設定した残り（システムはCSV Group列でなく livers.agency_id を所属の真実とする）。**livers.agency_id を CANDY へ修正済み → 6月CSVの再取込（上書き）で csv_data 再リンク＋分配recalc が走り確定**。全45名突合で不一致は彩愛1名のみ。教訓: ダミー用に変更した所属は実運用前に実CSVと突合して戻す。
...
> ⚠️ **訂正（2026-07-20・代表確認）**: 下図の「トータルサイドへ全額支払い」は会議メモの誤り（旧案）。
> **正: KIKIYOが各支払先（スカウト・マネージャー・代理店）へ個別に支払う**（ダミー計算資料③・実装と一致）。
> トータルサイドに集約するのは**スカウト分の報酬（請求書）だけ**（2026-06-24回答とも整合）。
> 分配明細の `total_side` 行は発注元（KIKIYO側）に残る残差であり、㈱トータルサイドへの支払いではない
> （同社への支払いはマネージャー杉谷40%行・スカウト杉谷行）。

```
KIKIYO（企業）
  └─ トータルサイド（統括）へ全額支払い　← ⚠️誤り・上記訂正参照
       └─ そこからマネージャー報酬を計算・分配
