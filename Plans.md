...
| **インボイス未登録の支払先に2%ロイヤリティ控除（請求書・分配明細）** | 🔲 コード実装・ローカル検証済（ブランチ `feat/invoice-royalty-2pct`）。**マイグレ046未適用・未デプロイ・未マージ** |

---

## 💴 インボイス未登録2%ロイヤリティ控除（2026-07-13 実装・未リリース）
...
- **リリース手順（未実施）**
  1. 本番へマイグレ046適用（runbook方式・単独で適用可）
  2. **適用直後にマスタ投入**: scouts/managers の登録番号が全員NULLのため、そのままだと全スカウト/MGに2%が掛かる。KIKIYO・杉谷陽生など**登録済みの支払先に `invoice_registration_number` を投入してから**当月を再計算すること（番号は発注元に確認）
  3. main へマージ→Vercel デプロイ
