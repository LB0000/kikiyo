...
### 残タスク（コードではなく運用）
- [x] **R2** 発注元に「ログアウト→再ログインを複数回」の再現確認を依頼 → ✅ 2026-06-02 依頼済み。**発注元の返答待ち**（再現しなければ要望#3クローズ。再現時はQ3-1〜Q3-6の切り分けへ）
...
### 残る不明点（Kouyama44 / kana / オーナーへ確認）
- [ ] 1人が複数ロール（例: キャンディーさんがスカウト10%＋担当40%）のとき、**同一人物への支払いを合算するか分離するか**。
- [ ] スカウト・マネージャーは**ログインできる主体**か、**集計上の区分だけ**か。
- [ ] 請求書は**分配先ごとに別発行**か、**1枚にまとめる**か。発行方向（誰→誰）。
- [ ] マネージャー↔ライバーの紐付けは**月ごとに変わりうるか**（固定マスタか月次更新か）。
- [ ] 各階層のパーセンテージの**確定値**と、育成要件で変える運用。
- [ ] 三次代理店の具体（クール配下）の分配率・請求単位。
- [ ] グループ名変更時の請求書名・見積もりへの影響（変更都度の運用）。
